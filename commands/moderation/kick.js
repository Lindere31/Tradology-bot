...
module.exports = {
    ...
    ownerOnly: false,
    usage: 'kick [@member] [reason] ou /kick [@member] [reason]',
    examples: ['kick @nom_utilisateur raison /kick @nom_utilisateur raison'],
    description: 'Kick un utilisateur avec une raison',
    async run(client, message, args) {
        if (!args[0]) return message.reply('Spécifier un membre à kick!');
        if (!args[1]) return message.reply('Spécifier un raison à votre kick!');
        ...
        target.kick(reason);
        message.channel.send(`Le membre ${target} à été kick!`);
    },
    options: [
        {
            name: 'target',
