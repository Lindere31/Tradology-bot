...
module.exports = {
    ...
    ownerOnly: false,
    usage: 'ban [@member] [reason] ou /ban [@member] [reason]',
    examples: ['ban @nom_utilisateur raison ou /ban @nom_utilisateur raison'],
    description: 'Bannir un utilisateur avec une raison',
    async run(client, message, args) {
        if (!args[0]) return message.reply('Spécifier un membre à ban!');
        if (!args[1]) return message.reply('Spécifier un raison à votre ban!');
        ...
        target.ban({ reason });
        message.channel.send(`Le membre ${target} à été ban!`);
    },
    options: [
        {
            name: 'target',
    ...
    async runInteraction(client, interaction) {
        ...
        target.ban({ reason });
        interaction.reply(`Le membre ${target} à été ban!`);
