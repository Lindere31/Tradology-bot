...
module.exports = {
    ...
    ownerOnly: false,
    usage: 'mute [@member] [duration] [reason] ou /mute [@member] [duration] [reason]',
    examples: ['mute @nom_utilisateur 4 minutes raison /mute @nom_utilisateur 4 minutes raison'],
    ...
    async run(client, message, args) {
        ...
        if (!args[3]) return message.reply('Spécifier un raison à votre mute!');
        ...
        message.channel.send(`Le membre ${target} à été mute pour ${duration} car ${reason}!`);
    },
    options: [
        {
            name: 'target',
        ...
        {
            name: 'duration',
            description: 'La durée du mute (ex: 10 minutes)',
            type: 'STRING',
            required: true,
        },
        {
    ...
    async runInteraction(client, interaction) {
        ...
        target.timeout(convertedTime, reason);
        interaction.reply(`Le membre ${target} à été mute pour ${duration} car ${reason}!`);
