module.exports = {
    ...
    ownerOnly: false,
    usage: 'thread [join|leave|archive|unarchive|delete] ou /thread [join|leave|archive|unarchive|delete]',
    examples: ['thread join, !thread leave ou /thread join, /thread leave'],
    description: 'Commande concernant les thread',
    async run(client, message, args) {
        ...
        if (!args[0] || !args[0].match(/^(join|leave|archive|unarchive|delete)$/)) 
            return message.reply(
                'Merci d\'entrer une sous-commande valide (\'join\'|\'leave\'|\'archive\'|\'unarchive\'|\'delete\')'
            );

        if (args[0] === 'join') {

            message.reply('Le bot a rejoind le thread!');
            if (thread.joinable) await thread.join();

        } else if (args[0] === 'leave') {

            message.reply('Le bot a quitté le thread!');
            if (thread.joinable) await thread.leave();

        } else if (args[0] === 'archive') {

            message.reply('Le thread est archivé!');
            if (thread.joinable) await thread.setArchived(true);

        } else if (args[0] === 'unarchive') {

            message.reply('Le thread est déarchivé!');
            if (thread.joinable) await thread.setArchived(false);
    ...
    options: [
        ...
        },
        {
            ...
            type: 'SUB_COMMAND',
            options: [{ name: 'channel', type: 'STRING', description: 'Id du channel ou le message de suppression va être envoyer', required: true }],
        },
    ],
    async runInteraction(client, interaction) {
        ...
        if (interaction.options.getSubcommand() === 'join') {
            ...
            interaction.reply('Le bot a rejoind le thread!');
            if (thread.joinable) await thread.join();
