...
module.exports = async (client) => {
  (await pGlob(`${process.cwd()}/commands/*/*.js`)).map(async (cmdFile) => {
    ...
    if (!cmd.permissions) return Logger.warn(`Commande non-chargée: ajouter une/des permission(s) à votre commande ↓\nFichier -> ${cmdFile}`);
    ...
    if (!cmd.examples) return Logger.warn(`Commande non-chargée: ajouter un/des exemple(s) (examples) à votre commande ↓\nFichier -> ${cmdFile}`);

      cmd.permissions.forEach(permission => {
        if (!permissionList.includes(permission)) {
          return Logger.typo(`Commande non-chargée: erreur de typo sur la permisson '${permission}' ↓\nFichier -> ${cmdFile}`);
        }
      });
